...
function RunCommand(msg, io, socket) {
    ...
    if (server.commands[commandName] != undefined) {
        ...
        msg.send = (content, serverMessage = false) => { io.in(msg.user.channel.name).emit('console', { serverMessage, user: msg.user.name, text: content }); };
        msg.reply = (content, serverMessage = false) => { socket.emit('console', { serverMessage, user: msg.user.name, text: content }); };
        msg.broadcast = (content, serverMessage = false) => { io.emit('console', { serverMessage, user: msg.user.name, text: content }); };
