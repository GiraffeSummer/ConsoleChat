...
const Load = (server) => {
    ...
    if (!fs.existsSync(eventPath)) {
        ...
        try {
            ...
            //go through all (default) event folders
...
function Subscribe(type, server) {
    //better error catching
    const eventPath = db.get('dataPath') + '/events';

    try {
        const evtType = type.toLowerCase();
        const eventFolderPath = eventPath + "/" + evtType;
        ...
        files.forEach((file) => {
            const event = require(eventFolderPath + `/${file}`);
            server.events[evtType].push(event);
            console.log(`Subscribed event ${file} for '${evtType}'`);
        });
    } catch (err) {
        console.log(`Error while subscribing events. ${err}`);
    }
...
function Run(type, args) {
    ...
    switch (type) {
        ...
        default:
            const passedData = Object.entries(args).map(([key, value]) => value);
            events.forEach(f => {
                f(...passedData);
            });
            break;
...
module.exports = { Run, Load, Subscribe }