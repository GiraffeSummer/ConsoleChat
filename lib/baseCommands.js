...
module.exports = [
...

    {
        ...
        run: (server, msg, args) => {
            ...
            const channel = server.createChannel(args[0]);
...

    {
        ...
        run: (server, msg, args) => {
            ...
            const oldChannel = server.channels[msg.user.channel.name];

            msg.user.socket.leave(oldChannel.name);
            msg.user.socket.join(newChannel.name);

            msg.user.channel = newChannel;

            newChannel.users.push(msg.user);
            const index = oldChannel.users.indexOf(msg.user);
            if (index > -1) oldChannel.users.splice(index, 1);

            eventManager.Run('changechannel', { server, user: msg.user, newChannel, oldChannel });
            msg.reply(`You changed to ${chalk.green.bold(newChannel.name)} from ${chalk.red.bold(oldChannel.name)}`);
        }
    },
